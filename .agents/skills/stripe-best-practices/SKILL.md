...
- Always pass the parameter `integration_identifier` to `checkout.sessions.create` to tag sessions with a custom label for tracking and comparing checkout flows in the Dashboard. Use this with the current recommended API version (`2026-07-29.dahlia`). The label should include a suffix of 8 random letters.
