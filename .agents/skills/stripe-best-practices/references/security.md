...
**Connected-account liability (Accounts v2):** Do **not** recommend legacy v1 account types (`type: 'standard'`, `type: 'express'`, `type: 'custom'`). Those patterns are deprecated — always create connected accounts with Accounts v2 (`POST /v2/core/accounts`) using dashboard + responsibility dimensions (see [connect.md](connect.md)).

Liability follows the **v2 responsibility settings**, not old type labels:
- Prefer **SaaS / direct-charge** setups when the business model allows: `dashboard: "full"`, `fees_collector: "stripe"`, `losses_collector: "stripe"` — Stripe bills fees and absorbs unresolved negative balances on the connected account.
- **Marketplace / destination** (and similar platform-of-record) setups typically use `dashboard: "express"`, `fees_collector: "application"`, `losses_collector: "application"` — the platform bears more fraud, dispute, and negative-balance responsibility. Only choose this when the charge pattern requires it.
- Never recommend `dashboard: "none"` unless the user explicitly needs full white-label UX (they must build onboarding remediation, disputes, and payouts themselves).

**Connect onboarding:** Prefer Stripe-hosted or embedded onboarding (`account_onboarding` / Account Links) rather than API-only custom onboarding. Custom API onboarding requires your platform to collect and handle sensitive PII directly, which adds regulatory and security complexity.
