...
- **`losses_collector: 'application'`** is compatible with all charge types when `fees_collector` is also `'application'`, with one exception: `full` dashboard + `application/application` is SALES-GATED (redirect to [Stripe sales](https://stripe.com/contact/sales)). Pairing `fees_collector: 'stripe'` with `losses_collector: 'application'` is BLOCKED for all charge types. `fees_collector: 'stripe'` + `losses_collector: 'stripe'` is ALLOWED only for `dashboard: 'full'` with **direct charges**; with `dashboard: 'none'`, that combo is BLOCKED for all charge types (see `compatibility-matrix.md`).
