...
export function useLiveSpectatorController() {
  ...
  const spectatorLifecycleRef = useRef(new LiveRoomLifecycle());
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const tok = await apiLiveToken(effectiveStreamId, isCoHost);
        if (tok.error || !mounted) {
          if (tok.error?.includes('401')) showToast('Please log in to watch');
          else if (tok.error?.includes('503')) showToast('Live video is not configured on server');
          return;
        }
        ...
        if (error || !session) {
          showToast(error || 'Could not connect to stream. Is the host live?');
          return;
        }
        const room = session.raw;
