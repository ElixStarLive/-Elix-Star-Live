...
export function useLiveHostController() {
  ...
  /** Synced from host LiveRoomLifecycle.rawRoom for tile attach helpers. */
  const liveKitRoomRef = useRef<Room | null>(null);
  const hostLifecycleRef = useRef(new LiveRoomLifecycle());
  ...
  const publishHostLiveKitTracks = useCallback(async () => {
    ...
    if (!stream || !hostLifecycleRef.current.liveKit?.connected) return;
    try {
      await hostLifecycleRef.current.publishFromStream(stream);
    } catch (e) {
    ...
    (async () => {
      try {
        ...
        if (!started.error && started.creds) {
          ...
          hostLifecycleRef.current.markHostRegistered();
          let url = started.creds.url.trim();
          if (!url) url = getLiveKitUrl();
          if (started.creds.token && url) {
            setLiveKitCreds({ token: started.creds.token, url });
          } else {
  ...
  useEffect(() => {
    ...
    const lifecycle = hostLifecycleRef.current;
    let cancelled = false;
    (async () => {
      const { error, session } = await lifecycle.connectLiveKitOnly(liveKitCreds, {
  ...
  const stopBroadcast = async () => {
    ...
    const { restEnded, error: endErr } = await hostLifecycleRef.current.endHostBroadcast(roomId);
    liveKitRoomRef.current = null;
    if (restEnded) {
      liveRegisteredRef.current = false;
  ...
  const recordLiveShareProgress = useCallback(() => {
    ...
    void request('/api/engagement/progress', {
      method: 'POST',
      body: JSON.stringify({ metric: 'shares', delta: 1, roomId: effectiveStreamId }),
    }).catch(() => {});
  }, [effectiveStreamId]);
