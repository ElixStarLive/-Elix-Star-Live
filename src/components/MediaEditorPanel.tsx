...
const TEXT_COLORS = ['#FFFFFF', '#000000', '#F12C56', '#D4AF37', '#00C2BE', '#7B5CFF', '#22C55E', '#F59E0B'];
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1">
            {FILTER_PRESETS.map((p) => (
              <button
                ...
                className={`shrink-0 px-3 py-2 rounded-xl text-xs font-semibold border ${activeFilterId === p.id ? 'bg-[#D4AF37] text-black border-[#D4AF37]' : 'bg-white/5 text-white/80 border-white/10'}`}
              >
                {p.label}
              </button>
            ))}
          ...
          <div className="flex gap-2.5 overflow-x-auto pb-1">
            {EFFECT_PRESETS.map((p) => (
              <button
                ...
                className={`shrink-0 px-3 py-2 rounded-xl text-xs font-semibold border ${activeEffectId === p.id ? 'bg-[#D4AF37] text-black border-[#D4AF37]' : 'bg-white/5 text-white/80 border-white/10'}`}
              >
                {p.label}
              </button>
            ))}
        ...
        {tab === 'text' && (
          <div>
            <input
              ...
              maxLength={120}
              autoFocus
              className="w-full bg-white/10 border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#D4AF37]/60 placeholder:text-white/30"
            />
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              {TEXT_COLORS.map((c) => (
                <button
                  ...
                  aria-label={`Color ${c}`}
                  className={`w-7 h-7 rounded-full border-2 ${textColor === c ? 'border-[#D4AF37]' : 'border-white/20'}`}
              ...
              <button
                type="button"
                onClick={submitText}
                className="ml-auto px-4 py-2 rounded-full bg-[#D4AF37] text-black text-xs font-bold active:scale-95 transition-transform"
              >
                Add
              </button>
            </div>
