...
export function CyclingRankBadge({ onOpen, className = '', showChevron = true }: Props) {
  ...
  return (
    <button
      type="button"
      className={`flex items-center gap-1 bg-black/75 rounded-full px-2.5 py-1 border border-[#5F0AE3]/80 shadow-[0_0_8px_rgba(95, 10, 227,0.35)] cursor-pointer active:scale-95 transition-transform ${className}`}
      onClick={(e) => {
        e.stopPropagation();
        onOpen(slot.tab);
      }}
      ...
      <span className="relative w-3.5 h-3.5 flex-shrink-0">
        {LIVE_RANK_CYCLE_SLOTS.map((s, i) => {
          ...
          return (
            <SlotIcon
              key={s.tab}
              className={`absolute inset-0 w-3.5 h-3.5 text-[#D2ADF8] transition-opacity duration-500 ${
