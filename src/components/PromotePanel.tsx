...
export default function PromotePanel({ isOpen, onClose, contentType, content }: PromotePanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100000] flex items-end justify-center bg-black/50" onClick={onClose}>
      <div
        className={`w-full max-w-[480px] bg-[#0B0B0E]/95 backdrop-blur-md rounded-t-2xl h-[38vh] max-h-[320px] overflow-hidden flex flex-col shadow-2xl animate-in slide-in-from-bottom ${!bottomNavHidden ? 'bottom-sheet-above-nav' : ''}`}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-3 min-h-0 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-[#5F0AE3]/50 [&::-webkit-scrollbar-thumb]:rounded-full">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                boostType === 'account' ? 'bg-[#5F0AE3] text-black' : 'bg-white/5 text-white/70'
              }`}
            >
              Boost account
            </button>
            {contentType === 'live' && (
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  boostType === 'live' ? 'bg-[#5F0AE3] text-black' : 'bg-white/5 text-white/70'
                }`}
              >
                Boost LIVE
              </button>
            ...
            {goals.map((g) => (
              <label
                ...
                className={`flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer transition-colors ${
                  selectedGoal === g.id ? 'bg-[#5F0AE3]/20 border border-[#5F0AE3]/40' : 'bg-white/5'
                }`}
              >
                <input type="radio" name="goal" checked={selectedGoal === g.id} onChange={() => setSelectedGoal(g.id)} className="sr-only" />
                <g.icon size={16} className="text-[#D2ADF8] flex-shrink-0" />
                <span className="text-white text-xs font-medium flex-1 truncate">{g.label}</span>
                {g.badge && <span className="text-[8px] font-bold text-[#D2ADF8] bg-[#5F0AE3]/20 px-1.5 py-0.5 rounded">New</span>}
              </label>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-white/5">
            <div className="w-9 h-9 rounded-lg overflow-hidden bg-[#0B0B0E] flex-shrink-0">
              {content?.thumbnail ? (
          ...
          <div className="flex items-center justify-between gap-2">
            <div>
              <p className="text-[#D2ADF8] text-lg font-bold">{estimates[selectedGoal] || estimates.likes}</p>
            ...
            <label className="flex items-center gap-2 px-2 py-1 rounded-lg bg-white/5 cursor-pointer shrink-0">
              ...
              <input type="radio" name="audience" checked={audience === 'default'} onChange={() => setAudience('default')} className="w-4 h-4 accent-[#FFFFFF]" />
            </label>
          </div>
        </div>

        {/* Bottom payment bar - compact */}
        <div className="flex items-center justify-between px-3 py-2 border-t border-white/10 bg-[#0B0B0E] flex-shrink-0">
          <div>
            {panelMessage ? (
          ...
          <button
            ...
            onClick={handlePay}
            disabled={isPaying}
            className="px-5 py-2 rounded-lg bg-[#5F0AE3] text-black font-bold text-xs hover:bg-[#5F0AE3]/90 active:scale-95 transition-all disabled:opacity-70"
          >
            {isPaying ? '...' : 'Pay'}
          </button>
        </div>
