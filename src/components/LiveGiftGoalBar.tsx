...
export function LiveGiftGoalBar({ goal, onTap, showSend = true }: Props) {
  ...
  return (
    <button
      type="button"
      onClick={onTap}
      className="pointer-events-auto flex items-center gap-2 max-w-[min(240px,72vw)] px-2 py-1.5 rounded-full bg-[rgba(10,10,10,0.72)] border border-[#D8D9DD]/35 backdrop-blur-md active:scale-[0.98] transition-transform shadow-lg"
    >
      {goal.giftIcon ? (
        <img
      ...
      <span className="text-[9px] font-bold text-[#F5F5F7] tabular-nums flex-shrink-0">
        {done ? "✓" : `${remaining}`}
      </span>
      {showSend && !done && (
        <span className="text-[8px] font-bold text-white bg-[#E6E9EE] px-1.5 py-0.5 rounded-full flex-shrink-0">
          Send
        </span>
      )}
    </button>
