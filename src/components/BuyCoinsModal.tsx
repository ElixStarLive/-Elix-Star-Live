      ...
      >
        <div className="bg-[#111111]/95 backdrop-blur-md rounded-t-2xl h-[40vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-1.5 px-4 pb-2 flex-shrink-0">
            <Coins className="w-3.5 h-3.5 text-[#D4AF37]" strokeWidth={1.8} />
              ...
              <div className="space-y-2">
                {nativeProducts.map((product) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg bg-white/[0.03] border border-white/10 hover:bg-[#C9A227]/10 transition-colors active:scale-[0.98] disabled:opacity-50"
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-[#111111] border border-[#C9A227]/30 flex items-center justify-center">
                    ...
                    <span className="text-[#D4AF37] text-[10px] font-bold">{nativeLoading === product.id ? 'Processing...' : `${product.coins} coins`}</span>
                  </button>
                ))}
              </div>
            ) : (
