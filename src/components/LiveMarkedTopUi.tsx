...
/** Photo 3D-style Neon Purple diamond for Diamond League. */
function LivePhotoDiamondIcon({ size = 14 }: { size?: number }) {
  const uid = `dl${size}`;
  return (
    <svg width={size} height={size} viewBox="0 0 18 18" className="flex-shrink-0 drop-shadow-[0_0_5px_rgba(199,125,255,0.7)]" aria-hidden>
      <defs>
      ...
      <path d="M9 7.4 L15.2 6.2 L9 16.8 Z" fill="#9D4EDD" opacity="0.9" />
      <path d="M9 7.4 L2.8 6.2 L9 16.8 Z" fill="#C77DFF" opacity="0.85" />
...
export function LiveExplorePill({ onOpen }: { onOpen: () => void }) {
  return (
    ...
    >
      <svg width="14" height="14" viewBox="0 0 12 12" className="flex-shrink-0 drop-shadow-[0_0_4px_rgba(199,125,255,0.7)]" aria-hidden>
        <defs>
        ...
        <path d="M6 1.2 C7.4 2.8 7.4 9.2 6 10.8 C4.6 9.2 4.6 2.8 6 1.2 Z" fill="#D9A8FF" opacity="0.35" />
      </svg>
      <span className="flex flex-col items-start justify-center leading-none min-w-0">
