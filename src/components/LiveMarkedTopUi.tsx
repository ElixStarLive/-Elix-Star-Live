...
export function LiveFollowPill({
  ...
  if (variant === 'photo') {
    return (
      <button
        type="button"
        className="flex items-center justify-center gap-0.5 h-[28px] px-2.5 rounded-full bg-[#DB0A3B] shadow-[0_0_10px_rgba(255,59,92,0.45)] active:scale-95 transition-transform flex-shrink-0"
        onClick={onFollow}
      >
  ...
  return (
    <button
      type="button"
      className="col-start-1 row-start-1 z-20 relative flex items-center justify-center gap-0.5 self-stretch h-full rounded-full bg-[#DB0A3B] w-full"
      onClick={onFollow}
    >
      <Plus size={12} className="text-white" strokeWidth={3} />
    ...
    <div className="flex items-center gap-1.5 min-w-0 pointer-events-auto">
      ...
      <div className="flex flex-col justify-center min-w-0 gap-[2px]">
        <div className="flex items-center gap-1 min-w-0">
          ...
          <BadgeCheck
            size={14}
            className="text-[#D2ADF8] flex-shrink-0"
            fill="#5F0AE3"
            stroke="#FFFFFF"
            strokeWidth={1.6}
          />
        ...
        <div className="flex items-center gap-1 mt-[1px]">
          ...
          <span
            className="inline-flex items-center gap-0.5 rounded-full px-1.5 py-[2px] text-[8px] font-bold text-white leading-none"
            style={{
              background: 'linear-gradient(90deg, #818CF8 0%, #D2ADF8 55%, #C4B5FD 100%)',
              border: '1px solid rgba(255,255,255,0.35)',
              boxShadow: 'none',
            }}
          >
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center gap-1 h-[28px] px-2.5 rounded-full active:scale-95 transition-transform ${
        hasJoinedToday
          ? 'bg-[#DB0A3B] border border-[#FF6B8A]'
          : 'bg-[#DB0A3B] border border-[#FF6B8A] shadow-[0_0_10px_rgba(255,59,92,0.55)]'
      }`}
      onClick={onJoin}
    >
      <div className="relative">
        ...
        />
        {!hasJoinedToday && (
          <div className="absolute -top-1 -right-1 w-2 h-2 bg-white rounded-full flex items-center justify-center border border-[#DB0A3B]">
            <span className="text-[#DB0A3B] text-[6px] font-bold leading-none">+</span>
          </div>
        )}
      </div>
    ...
    <svg width={size} height={size} viewBox="0 0 18 18" className="flex-shrink-0 drop-shadow-[0_0_6px_rgba(165,180,252,0.95)]" aria-hidden>
      <defs>
        ...
        <linearGradient id={`${uid}Bot`} x1="0.2" y1="0" x2="0.8" y2="1">
          <stop offset="0%" stopColor="#D2ADF8" />
      ...
      <path d="M9 7.4 L2.8 6.2 L9 16.8 Z" fill="#D2ADF8" opacity="0.85" />
