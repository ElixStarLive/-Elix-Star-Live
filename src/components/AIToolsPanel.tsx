...
export default function AIToolsPanel({
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[480px] bg-[#111113] rounded-t-2xl overflow-hidden animate-in slide-in-from-bottom duration-300" style={{ maxHeight: '70dvh' }}>
        
        {/* Header */}
        <div className="flex items-center justify-center px-4 py-3 ">
          <div className="flex items-center gap-2">
            <Wand2 size={18} className="text-[#B57CFF]" />
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${
                activeTab === tab.id
                  ...
                  : 'bg-[#111113] text-white/70 hover:text-white'
              }`}
            >
              {tab.icon}
              {tab.label}
                ...
                {FILTER_CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${
                      filterCategory === cat.id ? 'bg-[#6F2BFF]/20 text-[#B57CFF]' : 'bg-[#111113] text-white/50'
                    }`}
                  >
                    {cat.label}
                  </button>
                  ...
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${
                      selectedFilter === filter.id
                        ? 'bg-[#6F2BFF]/20 opacity-100'
                        : 'bg-[#111113] hover:bg-[#111113]/80'
                    }`}
                  >
          ...
          {activeTab === 'enhance' && (
            <div className="space-y-4">
              <div className="flex gap-2">
                <button onClick={handleAutoEnhance} className="flex-1 py-2 rounded-xl bg-[#6F2BFF] text-black text-xs font-bold flex items-center justify-center gap-1.5">
                  <Wand2 size={14} /> Auto Enhance
                </button>
                <button onClick={handleResetEnhance} className="flex-1 py-2 rounded-xl bg-[#111113] text-white/70 text-xs font-bold">
                  Reset
                </button>
              </div>
              {([
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-[#111113] text-white text-sm rounded-xl px-3 py-2 border border-white/10 resize-none h-20 outline-none focus:border-[#6F2BFF]/50"
                />
              </div>
              <button
                onClick={handleGenerateCaptions}
                className="w-full py-2.5 rounded-xl bg-[#6F2BFF] text-black text-xs font-bold flex items-center justify-center gap-1.5"
              >
                ...
                <div className="space-y-2">
                  ...
                  {captionSuggestions.map((s, i) => (
                    <button
                      ...
                      className="w-full text-left p-3 rounded-xl bg-[#111113] hover:bg-[#111113]/70 transition-colors"
                    >
                      <p className="text-white text-sm mb-1">{s.caption}</p>
                      <p className="text-[#B57CFF] text-xs">
                        {s.hashtags.map(h => `#${h}`).join(' ')}
                      </p>
                      <div className="flex items-center gap-1 mt-1">
                        <div className="h-1 flex-1 bg-white/10 rounded-full overflow-hidden">
                          <div className="h-full bg-[#FFFFFF]" style={{ width: `${s.score * 100}%` }} />
                        </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {generatedHashtags.map(tag => (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-full bg-[#6F2BFF]/10 text-[#B57CFF] text-xs font-medium hover:bg-[#6F2BFF]/20 transition-colors"
                      >
                        #{tag}
                      </button>
                    ))}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1 rounded-full bg-[#6F2BFF]/20 text-[#B57CFF] text-xs font-semibold"
                >
                  Refresh
                </button>
              </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {thumbnails.map((thumb, i) => (
                    ...
                    >
                      ...
                      {i === 0 && (
                        <div className="absolute top-1 left-1 px-1.5 py-0.5 rounded bg-[#6F2BFF] text-black text-[8px] font-bold">
                          BEST
                        </div>
                      )}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full bg-[#111113] text-white text-sm rounded-xl px-3 py-2.5 border border-white/10 outline-none appearance-none"
