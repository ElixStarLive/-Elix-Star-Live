  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="w-9 h-9 rounded-full bg-[#0B0B0E] backdrop-blur-md flex items-center justify-center shadow-lg active:scale-95 transition-transform relative"
        title="AI Filters"
      >
        <Sparkles size={16} className={`relative z-[2] ${currentFilter !== 'none' ? 'text-[#D2ADF8]' : 'text-white/70'}`} />
</button>
    );
  }
        ...
        <div className="bg-black/60 backdrop-blur-md rounded-2xl p-2" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center justify-center px-2 mb-2">
            <span className="text-white text-xs font-bold flex items-center gap-1">
              <Sparkles size={12} className="text-[#D2ADF8]" /> AI Filters
            </span>
          </div>
