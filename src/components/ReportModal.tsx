...
export default function ReportModal({ isOpen, onClose, videoId, contentType, contentId }: ReportModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 flex items-end justify-center" style={{ zIndex: 100100 }}>
      ...
      <div className="relative w-full max-w-[480px] z-10 elix-panel elix-live-sheet backdrop-blur-md rounded-t-2xl p-4 pb-safe flex flex-col gap-1 border border-black pointer-events-auto h-[40vh] max-h-[40vh] overflow-y-auto bottom-sheet-above-nav [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-white/5 [&::-webkit-scrollbar-thumb]:bg-[#E6E9EE]/50 [&::-webkit-scrollbar-thumb]:rounded-full" style={{ scrollbarWidth: 'thin', scrollbarColor: 'rgba(255,255,255,0.25) transparent' }}>
          ...
          {reportReasons.map((reason) => {
            ...
            return (
              ...
              >
                ...
                <div className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 ${selected ? 'border-[#D8D9DD] bg-[#E6E9EE]' : 'border-white/20'}`}>
                  {selected && (
