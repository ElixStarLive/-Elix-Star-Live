...
export default function LiveHostScreen() {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[480px] h-full bg-[#0B0B0E] overflow-hidden border-none">
        ...
        <div className="absolute inset-0 z-0 bg-[#0B0B0E] overflow-hidden">
        ...
        {!isBattleMode && (() => {
          ...
          >
            ...
            {isBroadcast || isBattleParticipant ? (
              <>
                ...
                {featuredHost && (
                  <>
                    ...
                    <div className="absolute top-1 left-1 z-20 flex items-center gap-1 pointer-events-auto">
                      ...
                      >
                        <X size={14} strokeWidth={2.35} className="text-[#D2ADF8]" />
                      </button>
                      <button
                        type="button"
                        title="Back to host on big screen"
                        onClick={(e) => { e.stopPropagation(); setFeaturedUserId(null); }}
                        className="flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-black/60 border border-[#5F0AE3]/50 active:scale-95"
                      >
                        <ArrowLeftRight className="w-3 h-3 text-[#D2ADF8]" strokeWidth={2.5} />
                        <span className="text-[8px] font-bold text-[#D2ADF8]">Host</span>
                      </button>
                    </div>
                ...
                {isCamOff && !featuredHost && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-[#0B0B0E] z-[5]">
                    {(user?.avatar || myAvatar) ? (
                      <img src={user?.avatar || myAvatar || ''} alt="" className="w-16 h-16 rounded-full border-2 border-[#5F0AE3]/40 object-cover object-center" />
                    ) : (
                      <div className="w-16 h-16 rounded-full border-2 border-[#5F0AE3]/40 bg-[#0B0B0E] flex items-center justify-center">
                        <span className="text-2xl font-black text-[#D2ADF8]/60">{(creatorName || user?.username || 'Me').charAt(0).toUpperCase()}</span>
                      </div>
                    )}
              ...
              <>
                ...
                />
                {!viewerHasStream && (
                  <div className="w-full h-full bg-[#0B0B0E] flex flex-col items-center justify-center relative">
                    {myAvatar ? (
                      <img src={myAvatar} alt="" className="w-28 h-28 rounded-full object-cover object-center mb-4 opacity-80" />
                    ) : (
                      <div className="w-28 h-28 rounded-full bg-[#0B0B0E] flex items-center justify-center mb-4">
                        <span className="text-4xl font-black text-[#D2ADF8]/60">{creatorName.charAt(0).toUpperCase()}</span>
                      </div>
                    )}
            ...
            {isBroadcast && cameraError && (
              <div className="absolute inset-0 flex items-center justify-center bg-[#0B0B0E] text-white font-bold">
                {cameraError}
              </div>
            )}
            </div>
              ...
              const renderCoHostCell = (slot: { type: 'host_main' | 'live' | 'invited' | 'pending' | 'empty'; host?: (typeof coHosts)[0] }) => {
                  ...
                  return (
                    <>
                      <video
                        ...
                        controls={false}
                        poster={LIVE_VIDEO_TRANSPARENT_POSTER}
                        style={{ opacity: isCamOff ? 0 : 1, transform: 'scaleX(-1)', backgroundColor: '#0B0B0E' }}
                      />
                      {isCamOff && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 bg-[#0B0B0E] z-[5]">
                          {(user?.avatar || myAvatar) ? (
                            <img src={user?.avatar || myAvatar || ''} alt="" className="w-10 h-10 rounded-full object-cover" />
                          ) : (
                            <div className="w-10 h-10 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                              <span className="text-[#D2ADF8]/60 text-sm font-bold">{(creatorName || 'Me').charAt(0)}</span>
                            </div>
                          )}
                        </div>
                      )}
                      <div className="absolute top-0.5 left-0.5 z-10 flex items-center gap-0.5 pointer-events-auto">
                        ...
                        <button
                          ...
                          onClick={(e) => { e.stopPropagation(); setFeaturedUserId(null); }}
                          className="rounded bg-black/55 p-0.5 border border-[#5F0AE3]/45 active:scale-95"
                        >
                          <ArrowLeftRight className="w-3 h-3 text-[#D2ADF8]" strokeWidth={2.5} />
                        </button>
                      </div>
                    ...
                    <>
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 bg-[#0B0B0E] z-[5]">
                        {host.avatar ? (
                          ...
                          <div className="w-10 h-10 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                            <span className="text-[#D2ADF8]/60 text-sm font-bold">{(host.name || '?').charAt(0)}</span>
                          </div>
                        )}
                      ...
                      {(lastGiftIcon || score > 0) && (
                        <div className="absolute bottom-0.5 right-0.5 z-10 flex items-center pointer-events-none">
                          {lastGiftIcon && (
                            <div className="w-5 h-5 rounded-full bg-[#0B0B0E] border border-[#5F0AE3]/40 overflow-hidden flex items-center justify-center drop-shadow-md z-10 relative">
                              <img src={lastGiftIcon} alt="gift" className="w-full h-full object-cover" />
                            </div>
                          )}
                          {score > 0 && (
                            <div
                              className={`h-4 flex items-center rounded-full text-[8px] font-bold text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] relative z-0 bg-[#0B0B0E]/40 backdrop-blur-md border border-white/10 ${lastGiftIcon ? '-ml-2 pl-3 pr-1.5' : 'px-1.5'}`}
                            >
                      ...
                      {isSelected && (
                        <div className="absolute inset-0 z-[5] pointer-events-none border-2 border-[#5F0AE3]" />
                      )}
                    </>
                  );
                }
                if (slot.type === 'invited' && slot.host) return (
                  <>
                    ...
                    <div className="w-12 h-12 rounded-full overflow-hidden bg-[#0B0B0E]">
                      {slot.host.avatar ? <img src={slot.host.avatar} alt="" className="w-full h-full object-cover opacity-60" /> : <div className="w-full h-full flex items-center justify-center text-[#D2ADF8]/60 text-base font-bold">{(slot.host.name || '?').charAt(0)}</div>}
                    </div>
                    <p className="text-white/60 text-[9px] font-bold mt-0.5 truncate max-w-[95%] text-center">{slot.host.name}</p>
                    <span className="text-[#D2ADF8]/70 text-[8px] font-semibold">Waiting</span>
                  </>
                );
                if (slot.type === 'pending' && slot.host) return (
                  <>
                    ...
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-[#0B0B0E]">
                      {slot.host.avatar ? <img src={slot.host.avatar} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-[#D2ADF8] text-sm font-bold">{(slot.host.name || '?').charAt(0)}</div>}
                    </div>
                    <p className="text-white text-[8px] font-bold mt-0.5 truncate max-w-[95%] text-center">{slot.host.name}</p>
                    <span className="text-[#D2ADF8]/70 text-[8px] font-semibold">Pending</span>
                  </>
                );
                return (
                  ...
                  {smallSlots.slice(0, 8).map((slot, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={`relative bg-[#0B0B0E] flex flex-col items-center justify-center overflow-hidden p-0 min-h-0 border border-[#C9A96E]/40 ${cellSpeaking ? 'elix-speaking-pulse' : ''} ${cellHost && !isBattleMode ? 'cursor-pointer' : ''}`}
                      >
          ...
          >
            ...
            {SPEED_CHALLENGE_ENABLED && speedChallengeResult && !speedChallengeActive && (
              <div className="absolute inset-x-0 bottom-24 z-[270] pointer-events-none flex items-center justify-center">
                <div className="flex flex-col items-center gap-1 px-6 py-3 rounded-xl bg-[#0B0B0E]/70 backdrop-blur-md border border-white/15 shadow-[0_0_20px_rgba(0,0,0,0.6)]">
            ...
            {(() => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`relative z-20 w-full flex-none ${battleScoreBarHidden ? '' : 'bg-[#0B0B0E]/95 border-b border-white/10'}`}>
                    {!battleScoreBarHidden ? (
                      <div
                      ...
                      >
                        <div className="absolute inset-0 flex">
                          <div
                            ...
                            style={{ width: `${leftPct}%`, backgroundImage: 'linear-gradient(90deg, #DB0A3B, #FF1744, #C41E3A)' }}
                          />
                      ...
                      >
                        <div className="relative w-5 h-5 flex items-center justify-center flex-shrink-0">
                          <svg viewBox="0 0 40 44" className="absolute inset-0 w-full h-full drop-shadow-md">
                            ...
                            <defs><linearGradient id="vsGrad2" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#DB0A3B"/><stop offset="50%" stopColor="#8B0000"/><stop offset="100%" stopColor="#1E90FF"/></linearGradient></defs>
                          </svg>
                  ...
                  <div ref={battleVoteGridRef} className="flex-1 min-h-0 flex flex-col relative">
                    ...
                    <div className="flex flex-1 min-h-0 gap-0">
                      <div
                        className="flex-1 basis-0 min-w-0 h-full overflow-hidden relative bg-[#0B0B0E] pointer-events-auto"
                      >
                      ...
                      {isCamOff && (
                        <div className="absolute inset-0 z-[5] flex flex-col items-center justify-center gap-1 bg-[#0B0B0E]">
                          {(user?.avatar || myAvatar) ? (
                            <img src={user?.avatar || myAvatar || ''} alt="" className="w-12 h-12 rounded-full object-cover object-center" />
                          ) : (
                            <div className="w-12 h-12 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                              <span className="text-lg font-black text-[#D2ADF8]/60">{(creatorName || user?.username || 'Me').charAt(0).toUpperCase()}</span>
                            </div>
                          )}
                    ...
                    >
                      {battleSlots[0].status === 'accepted' ? (
                        <div className="w-full h-full relative bg-[#0B0B0E]">
                          ...
                          {cameraOffPlayers['opponent'] && (
                            <div className="absolute inset-0 z-[11] flex flex-col items-center justify-center gap-2 bg-[#0B0B0E]">
                              {battleSlots[0].avatar ? (
                                <img src={battleSlots[0].avatar} alt="" className="w-16 h-16 rounded-full object-cover object-center" />
                              ) : (
                                <div className="w-16 h-16 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                                  <span className="text-2xl font-black text-[#D2ADF8]/60">{(battleSlots[0].name || 'P').charAt(0).toUpperCase()}</span>
                                </div>
                              )}
                          ...
                          {!hasOpponentStream && !cameraOffPlayers['opponent'] && (
                            <div className="absolute inset-0 z-[5] flex flex-col items-center justify-center gap-2 bg-[#0B0B0E]">
                              {battleSlots[0].avatar ? (
                                <img src={battleSlots[0].avatar} alt={battleSlots[0].name} className="w-16 h-16 rounded-full object-cover object-center" />
                              ) : (
                                <div className="w-16 h-16 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                                  <span className="text-2xl font-black text-[#D2ADF8]">{(battleSlots[0].name || 'P').charAt(0).toUpperCase()}</span>
                                </div>
                              )}
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-[#0B0B0E]">
                          ...
                          <div className="w-5 h-5 border-2 border-[#5F0AE3] border-t-transparent rounded-full animate-spin" />
                          <span className="text-white text-[10px] font-bold">Waiting...</span>
                        </div>
                      ) : (
                        <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-[#0B0B0E]/80 pointer-events-auto" onClick={(e) => { e.stopPropagation(); setShowViewerList(false); setIsFindCreatorsOpen(true); }}>
                          <div className="w-12 h-12 rounded-full flex items-center justify-center">
                        ...
                        <>
                          ...
                          <div className="absolute top-3 right-1.5 z-10 pointer-events-auto">
                            <button
                              ...
                              className="flex flex-col items-center gap-0.5 border-0 bg-transparent p-0 hover:opacity-90 active:scale-95"
                            >
                              <X size={14} className="text-[#D2ADF8]" strokeWidth={2.25} />
                            </button>
                          </div>
                      ...
                      >
                        ...
                        <div 
                          className={`h-4 flex items-center rounded-full text-[8px] font-bold text-white drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] relative z-0 bg-[#0B0B0E]/40 backdrop-blur-md border border-white/10 ${lastGifts.opponent ? '-ml-2 pl-3 pr-1.5' : 'px-1.5'}`}
                        >
                        ...
                        {battleSlots[1].status === 'accepted' ? (
                          <div className="w-full h-full relative bg-[#0B0B0E]">
                            ...
                            {cameraOffPlayers['player3'] && (
                              <div className="absolute inset-0 z-[11] flex flex-col items-center justify-center gap-1 bg-[#0B0B0E]">
                                {battleSlots[1].avatar ? (
                                  <img src={battleSlots[1].avatar} alt="" className="w-12 h-12 rounded-full object-cover object-center" />
                                ) : (
                                  <div className="w-12 h-12 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                                    <span className="text-lg font-black text-[#D2ADF8]/60">{(battleSlots[1].name || '?').charAt(0).toUpperCase()}</span>
                                  </div>
                                )}
                            ...
                            {cameraOffPlayers['player4'] && (
                              <div className="absolute inset-0 z-[11] flex flex-col items-center justify-center gap-1 bg-[#0B0B0E]">
                                {battleSlots[2].avatar ? (
                                  ...
                                  <div className="w-12 h-12 rounded-full bg-[#0B0B0E] flex items-center justify-center">
                                    <span className="text-lg font-black text-[#D2ADF8]/60">{(battleSlots[2].name || '?').charAt(0).toUpperCase()}</span>
                                  </div>
                                )}
            ...
            <div className="absolute bottom-1 left-0 right-0 px-3 py-2 flex items-center justify-between flex-none pointer-events-none relative z-30" style={{ transform: 'translateY(1mm)' }}>
              ...
              >
                {topMvpHostBattle.map((viewer, i) => {
                  ...
                  >
                    <div className={isMvp ? 'rounded-full ring-2 ring-[#5F0AE3] p-[1px] shadow-[0_0_6px_rgba(95, 10, 227,0.55)]' : 'rounded-full'}>
                      <AvatarRing
                    ...
                    {isMvp && (
                      <span className="absolute top-[22px] left-1/2 -translate-x-1/2 z-[2] px-1 rounded-full bg-[#5F0AE3] text-black text-[6px] font-black leading-none tracking-wide">
                        MVP
                      </span>
                    )}
                    ...
                    <span className="text-[#D2ADF8] text-[7px] font-black tabular-nums leading-none drop-shadow-[0_1px_2px_rgba(0,0,0,0.95)]">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="pointer-events-auto flex flex-col gap-2">
                        ...
                        <div className="px-0 py-1 animate-luxury-fade-in relative">
                          ...
                          />
                          {currentUniverse && (
                            <div className="mt-1 flex items-center gap-1 bg-[#0B0B0E]/90 rounded-full px-2.5 py-1 border border-[#5F0AE3]/80 shadow-sm pointer-events-auto relative z-20">
                      ...
                      <div className="pointer-events-auto flex items-center gap-[0mm] mt-1">
                          ...
                          >
                            {topMvpViewers.slice(0, 1).map((viewer) => {
                              ...
                              >
                                ...
                                {isMvp && (
                                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 z-[2] px-1 rounded-full bg-[#5F0AE3] text-black text-[6px] font-black leading-none tracking-wide">
                                    MVP
                                  </span>
                                )}
                              </div>
                        ...
                        >
                          ...
                          <UserPlus size={16} className="text-[#D2ADF8]" strokeWidth={2.2} />
                        </button>
                        <button
                          type="button"
...
{/* BOTTOM RIGHT: Action buttons (same area as before, aligned right) */}
          ...
          {!isCreatorParticipant && (
            <div className="flex items-end gap-2 w-full max-w-[480px] pointer-events-auto">
              <form className="flex-1 flex items-center gap-2 bg-black/40 backdrop-blur-sm rounded-full px-3 py-2 border border-white/10 h-10 min-w-0" onSubmit={(e) => { e.preventDefault(); handleSendMessage(e); }}>
                ...
                {inputValue.trim() && <button type="submit" title="Send message" className="text-[#D2ADF8] flex-shrink-0"><Send size={16} /></button>}
              </form>
              <div className="flex flex-col items-center gap-0.5">
                ...
                >
                  <BarChart3 size={20} className="text-[#5F0AE3] relative z-[2]" strokeWidth={2.2} />
                </button>
              ...
              >
                <span className="flex items-center justify-center w-full h-full relative z-[2]"><UserPlus size={20} className="text-[#D2ADF8] shrink-0" strokeWidth={2} /></span>
</button>
              <button type="button" title="Send gift" onClick={openGiftPanel} className={`${LIVE_BOTTOM_ICON_BTN} relative`}>
                <Gift size={20} className="text-[#D2ADF8] relative z-[2]" />
</button>
              <button type="button" title="Share" onClick={openSharePanel} className={`${LIVE_BOTTOM_ICON_BTN} relative`}>
                <Share2 size={20} className="text-[#D2ADF8] relative z-[2]" />
</button>
              <button type="button" title="More options" onClick={openMoreMenu} className={`${LIVE_BOTTOM_ICON_BTN} relative`}>
                <MoreVertical size={20} className="text-[#D2ADF8] relative z-[2]" />
</button>
            </div>
          )}
            ...
            <div className="flex items-end gap-2 w-full max-w-[480px] pointer-events-auto">
              ...
              {isBattleMode && battleWinner && isBroadcast && (
                <button 
                  ...
                  className="px-4 h-10 rounded-full bg-[#0B0B0E] backdrop-blur-md flex items-center justify-center shadow-lg active:scale-95 transition-transform"
                >
                  <RefreshCw size={20} className="text-[#D2ADF8] mr-2" />
                  <span className="text-[#D2ADF8] text-xs font-bold">Rematch</span>
                </button>
              )}
              ...
              <div className="flex flex-col items-center gap-0.5">
                <button
                  ...
                  className={`${LIVE_BOTTOM_ICON_BTN} relative`}
                >
                  <Users size={20} className="text-[#D2ADF8] relative z-[2]" />
</button>
      ...
      {isFindCreatorsOpen && (
        <div className="fixed inset-0 z-[99999] flex flex-col justify-end max-w-[480px] mx-auto" style={{ height: '100%' }}>
          ...
          />
          <div
            className="bg-[#0B0B0E]/95 backdrop-blur-md rounded-t-2xl h-[40vh] flex flex-col shadow-2xl pointer-events-auto w-full relative z-10 overflow-hidden pb-safe"
              ...
              <div className="space-y-1 pb-4">
                ...
                {filteredCreators.length === 0 && creatorsLoading ? (
                  <div className="py-6 flex justify-center">
                    <div className="w-5 h-5 border-2 border-[#5F0AE3]/40 border-t-transparent rounded-full animate-spin" />
                  </div>
                ) : null}
                {filteredCreators.length === 0 && creatorsLoadFailed ? (
                  <div className="py-6 flex justify-center">
                    <button type="button" onClick={() => loadCreators()} className="px-3 py-1.5 rounded-lg bg-[#5F0AE3]/20 border border-[#5F0AE3]/40 text-[#D2ADF8] text-[10px] font-bold active:scale-95">
                      Retry
                    </button>
                  </div>
                ) : null}
            ...
            {isBroadcast && battleSlots.some(s => s.status === 'accepted') && (
              <div className="px-4 py-2 flex-shrink-0">
                <button
                  ...
                  className="w-full py-2.5 bg-[#5F0AE3] text-black text-xs font-bold rounded-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-1.5"
                >
                  <Sword size={14} />
      ...
      <AnimatePresence>
        {miniProfile && (
          <div className="absolute inset-0 z-[10000] flex flex-col justify-end">
            ...
            />
            <motion.div
              className="bg-[#0B0B0E] rounded-t-2xl border-t border-white/10 px-4 pt-4 pb-[calc(20px+env(safe-area-inset-bottom))] pointer-events-auto shadow-2xl relative z-10"
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
                  ...
                  <div className="min-w-0 pt-1">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      {miniProfile?.id && moderators.has(miniProfile.id) && (
                        <User className="w-3.5 h-3.5 text-[#D2ADF8] flex-shrink-0" strokeWidth={2.25} aria-hidden />
                      )}
                      <LevelBadge
                ...
                <button
                  ...
                  className={`h-9 rounded-lg text-[11px] active:scale-95 transition-all ${
                    ...
                    (miniProfileFollowsThem === true ||
                      ...
                      : 'bg-[#5F0AE3] text-black font-black hover:bg-[#5F0AE3]/90'
                  }`}
                >
                  {miniProfile?.id &&
                  (miniProfileFollowsThem === true ||
      ...
      {showViewerList && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[999999] pointer-events-auto max-w-[480px] mx-auto">
            <div className="bg-[#0B0B0E]/95 backdrop-blur-md rounded-t-2xl h-[36vh] flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar px-4 pb-4 min-h-0">
                {viewerListMode === 'topGifters' ? (
                      ...
                      topGiftersForPanel.map((v, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-[#D2ADF8] text-xs font-bold tabular-nums flex-shrink-0">
                  ...
                  <>
                {pendingInvite && (
                  <div className="mb-3 flex items-center gap-2.5 w-full py-1 px-2 rounded-full bg-[#5F0AE3]/10 border border-[#5F0AE3]/30">
                    <div
                      className="rounded-full overflow-hidden bg-[#0B0B0E] flex-shrink-0"
                    ...
                    >
                      {pendingInvite.hostAvatar ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[#D2ADF8] font-bold">{pendingInvite.hostName.slice(0, 1).toUpperCase()}</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                  ...
                  <div className="mb-3 flex items-center gap-2.5 w-full py-1 px-2 rounded-full bg-[#5F0AE3]/10 border border-[#5F0AE3]/30">
                    ...
                    >
                      {pendingCohostInvite.hostAvatar ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-[#D2ADF8] font-bold">{pendingCohostInvite.hostName.slice(0, 1).toUpperCase()}</div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
      ...
      {showJoinAnimation && (
        <div className="absolute inset-0 z-[99999] flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center animate-in zoom-in-50 duration-300">
            ...
            />
            <Heart className="w-20 h-20 text-[#D2ADF8] fill-[#ffffff] drop-shadow-2xl animate-pulse hidden" />
      ...
      {showTeamStatus && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 h-[40vh] z-[99999] pointer-events-auto max-w-[480px] mx-auto">
          <div
            className="bg-[#0B0B0E]/95 backdrop-blur-md rounded-t-2xl p-3 pb-safe h-full flex flex-col shadow-2xl w-full overflow-hidden "
            onClick={(e) => e.stopPropagation()}
          >
            {/* Drag handle */}
            ...
            <div className="flex items-center justify-between px-4 pb-2 flex-shrink-0">
              <div className="flex items-center gap-1.5">
                <Heart className="w-3 h-3 text-[#D2ADF8]" strokeWidth={2} fill="#5F0AE3" />
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 no-scrollbar min-h-0">
               {/* Team Status Card */}
               <div className="bg-white/5 rounded-xl p-3 border border-[#5F0AE3]/20 relative overflow-hidden">
                 <div className="flex items-center gap-3 relative z-10">
                   <div 
                     className="w-10 h-10 rounded-full bg-gradient-to-br from-[#D2ADF8] to-[#D2ADF8] flex items-center justify-center shadow-lg cursor-pointer active:scale-95 transition-transform"
                     onClick={(e) => {
                       e.stopPropagation();
                   ...
                   <div>
                     <div className="text-[#D2ADF8]/60 text-[9px] font-bold uppercase tracking-wider">Member Hearts</div>
               ...
               <div className="mt-3">
                 <h4 className="text-[#D2ADF8]/60 text-[9px] font-bold uppercase tracking-wider mb-2 px-1">Hearts Sent</h4>
                 <div className="space-y-1">
                   ...
                   {heartMembers.map((m, i) => (
                     <div key={m.user_id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-[#5F0AE3]/5 border border-[#5F0AE3]/15">
                       <div className="w-5 text-center font-bold text-[10px] text-[#D2ADF8]/60">{i + 1}</div>
                       <img src={m.avatar_url || '/royce/elix-mark.svg'} alt="" className="w-7 h-7 rounded-full object-cover border border-[#5F0AE3]/20" />
                       <div className="flex-1 min-w-0">
                         <div className="text-[10px] font-bold text-white truncate">{m.username || m.user_id.slice(0, 8)}</div>
                       </div>
                       <div className="text-[#D2ADF8] text-[10px] font-bold whitespace-nowrap">
               ...
               {/* Top Gifters */}
               <div className="mt-3">
                 <h4 className="text-[#D2ADF8]/60 text-[9px] font-bold uppercase tracking-wider mb-2 px-1">Top Supporters</h4>
                 <div className="space-y-1">
                   {topGifters.length === 0 && (
                     <p className="text-white/30 text-[10px] text-center py-2">No gifts yet</p>
                   )}
                   {topGifters.map((g, i) => (
                     <div key={g.user_id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-[#5F0AE3]/5 border border-[#5F0AE3]/15">
                       <div className="w-5 text-center font-bold text-[10px] text-[#D2ADF8]/60">{i + 1}</div>
                       <img src={g.avatar_url || '/royce/elix-mark.svg'} alt="" className="w-7 h-7 rounded-full object-cover border border-[#5F0AE3]/20" />
                       <div className="flex-1 min-w-0">
      ...
      {showFanClub && (
        <>
          ...
          <div
            className="bg-[#0B0B0E]/95 rounded-t-2xl p-3 pb-safe max-h-[40vh] overflow-y-auto no-scrollbar shadow-2xl w-full"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Drag handle */}
            <div className="flex justify-center pt-3 pb-1">
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 no-scrollbar">
              <div className="flex flex-col gap-3">
                {/* Subscription Banner */}
                <div className="bg-gradient-to-r from-[#D2ADF8]/10 to-[#B8943F]/5 rounded-xl p-3 border border-[#5F0AE3]/20 relative overflow-hidden">
                  <div className="relative z-10">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="w-6 h-6 bg-[#5F0AE3]/20 rounded-full flex items-center justify-center border border-[#5F0AE3]/30">
                        <Heart className="w-2.5 h-2.5 text-[#D2ADF8] fill-[#FFFFFF] animate-pulse" />
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-gradient-to-r from-[#D2ADF8] to-[#D2ADF8] text-black font-bold text-[10px] uppercase tracking-wide rounded-xl active:scale-[0.98] transition-all shadow-lg disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                    >
                      {isSubscribing ? (
                        <>
                ...
                <div className="bg-white/5 rounded-xl p-3 border border-[#5F0AE3]/20">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-gold-metallic font-bold text-[10px] flex items-center gap-1">
                      <div className="w-4 h-4 rounded-full bg-[#0B0B0E] flex items-center justify-center border border-[#5F0AE3]/40">
                    ...
                    <span className="bg-[#5F0AE3]/10 text-[#D2ADF8] text-[7px] font-bold px-1.5 py-0.5 rounded-full border border-[#5F0AE3]/20">
                  ...
                  <div className="grid grid-cols-4 gap-1.5">
                    {creatorStickers.map((sticker) => (
                      <div key={sticker.id} className="aspect-square rounded-lg bg-white/5 border border-[#5F0AE3]/10 relative overflow-hidden group">
                    ...
                    {creatorStickers.length < 20 && (
                      <button
                        className="aspect-square rounded-lg bg-white/5 hover:bg-white/10 active:scale-95 transition-all flex items-center justify-center border border-dashed border-[#5F0AE3]/30 relative overflow-hidden"
                      ...
                      >
                        {stickerUploading ? (
                          ...
                          <div className="flex flex-col items-center gap-0.5">
                            <PlusCircle size={14} className="text-[#D2ADF8]/60" />
                            <span className="text-[6px] text-[#D2ADF8]/60 font-bold uppercase">Upload</span>
                          </div>
                        )}
                      </button>
                    )}
      ...
      {isMoreMenuOpen && (
        <>
          ...
          >
          <div
            className="relative bg-[#0B0B0E]/95 rounded-t-2xl p-3 pb-safe h-[40vh] overflow-y-auto no-scrollbar shadow-2xl w-full "
            onClick={(e) => e.stopPropagation()}
          >
            {areTestCoinsEnabled() && (
              <button
            ...
            <div className="grid grid-cols-4 gap-y-4 gap-x-2 pt-1 pb-2 px-1">
              ...
              <button type="button" onClick={moreShare} className="!flex !flex-col !items-center !justify-start gap-1.5 w-full active:scale-95 transition-transform">
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  <Share2 className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />
                </div>
              ...
              >
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  <Gift className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />
                </div>
              ...
              <button type="button" disabled={!isBroadcast} onClick={moreFlipCamera} className="!flex !flex-col !items-center !justify-start gap-1.5 w-full active:scale-95 transition-transform disabled:opacity-40">
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  <RefreshCw className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />
                </div>
              ...
              <button type="button" disabled={!isBroadcast} onClick={moreToggleMic} className="!flex !flex-col !items-center !justify-start gap-1.5 w-full active:scale-95 transition-transform disabled:opacity-40">
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  {isMicMuted ? <MicOff className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} /> : <Mic className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />}
                </div>
              ...
              <button type="button" disabled={!isBroadcast} onClick={moreToggleCam} className="!flex !flex-col !items-center !justify-start gap-1.5 w-full active:scale-95 transition-transform disabled:opacity-40">
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  {isCamOff ? <CameraOff className="w-[18px] h-[18px] text-white/60 relative z-[2]" strokeWidth={1.8} /> : <Camera className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />}
                </div>
              ...
              >
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  <Sparkles className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />
                </div>
              ...
              <button type="button" onClick={moreToggleChat} className="!flex !flex-col !items-center !justify-start gap-1.5 w-full active:scale-95 transition-transform">
                <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                  <MessageCircle className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />
                </div>
              ...
              {isBroadcast && (
                <>
                  ...
                  {([5, 10, 15] as const).map((mins) => (
                    ...
                    >
                      <div className="royce-glow-disc w-11 h-11 rounded-full relative !flex !items-center !justify-center shrink-0">
                        <Timer className="w-[18px] h-[18px] text-[#D2ADF8] relative z-[2]" strokeWidth={1.8} />
                      </div>
      ...
      {showLiveEffectsPanel && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[99999] pointer-events-auto max-w-[480px] mx-auto">
            <div
              className="bg-[#0B0B0E]/95 rounded-t-2xl p-3 pb-safe shadow-2xl w-full"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-center mb-2">
                <div className="w-10 h-1 bg-white/20 rounded-full" />
              ...
              <div className="flex items-center justify-center gap-1.5 mb-2">
                <Sparkles size={14} className="text-[#D2ADF8]" />
      ...
      {areTestCoinsEnabled() && showTestCoinsModal && (
        <>
          ...
          >
            <div
              className="bg-[#0B0B0E] rounded-2xl p-5 mx-6 w-full max-w-xs shadow-2xl border border-[#5F0AE3]/30 pointer-events-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center gap-2 mb-4">
                <Lock className="w-5 h-5 text-[#D2ADF8]" />
              ...
              {testCoinsStep === 'password' && (
                ...
                >
                  <input
                    ...
                    className="w-full bg-[#0B0B0E] text-white text-sm rounded-xl px-4 py-3 border border-white/10 focus:border-[#5F0AE3]/60 focus:outline-none placeholder:text-white/30 mb-2"
                  />
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 rounded-xl bg-[#5F0AE3] text-black text-sm font-bold disabled:opacity-40"
                    >
                      Unlock
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 mb-3">
                    {[1000, 5000, 10000, 25000, 50000, 100000].map(amt => (
                      <button
                        ...
                        className="py-1.5 rounded-lg text-xs font-bold transition-colors bg-white/5 text-white/70 hover:bg-[#5F0AE3]/20"
                      >
                    ...
                    <button
                      ...
                      className="py-1.5 rounded-lg text-xs font-bold transition-colors bg-[#5F0AE3]/30 text-[#D2ADF8] hover:bg-[#5F0AE3]/40 col-span-3"
                    >
      ...
      {boosterActivations.length > 0 && (
        <div className="fixed left-3 top-[92px] z-[100000] flex flex-col gap-1 pointer-events-none">
          {Object.values(
            ...
            <span key={g.key} className="relative flex items-center justify-center w-11 h-11 rounded-full bg-[#0B0B0E]/90 border border-[#FF3B30] shadow-2xl text-[#FF3B30] animate-in zoom-in-50 duration-200">
      ...
      {boosterCatches.length > 0 && (
        <div className="fixed inset-x-0 top-[30%] z-[100000] flex flex-col items-center gap-2 pointer-events-none px-4">
          {boosterCatches.map((c) => (
            <div key={c.id} className="booster-catch-pop flex items-center gap-2 px-4 py-2 rounded-full bg-[#0B0B0E]/90 border border-[#5F0AE3] shadow-2xl">
              <GloveIcon className="w-5 h-5 text-[#D2ADF8]" />
              <span className="text-[#D2ADF8] font-black text-base tracking-wide">x{c.multiplier} CAUGHT!</span>
      ...
      {showSharePanel && (
        <>
          ...
          />
          <div className="fixed bottom-0 left-0 right-0 z-[99999] pointer-events-auto max-w-[480px] mx-auto">
          <div className="bg-[#0B0B0E]/95 backdrop-blur-md rounded-t-2xl p-3 pb-safe flex flex-col shadow-2xl w-full h-[40vh] overflow-hidden ">
            ...
            <div className="mx-4 my-1 border-t border-[#5F0AE3]/45 flex-shrink-0" aria-hidden />

            {/* Share options — same layout as ShareModal */}
            <div className="flex-1 overflow-y-scroll overflow-x-hidden min-h-0 px-4 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-white/5 [&::-webkit-scrollbar-thumb]:bg-[#5F0AE3]/60 [&::-webkit-scrollbar-thumb]:rounded-full">
      ...
      {showModerationWarning && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/70" onClick={() => { setShowModerationWarning(false); setModerationWarningMessage(''); }}>
          <div className="bg-[#0B0B0E] border border-white/10 rounded-xl p-6 max-w-sm w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="w-full py-2.5 rounded-lg bg-[#5F0AE3] text-black font-semibold"
            >
              OK
            </button>
          </div>
