  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#111113] flex flex-col max-w-[480px] mx-auto">
      <div className="flex items-center justify-between px-3 pt-[max(12px,env(safe-area-inset-top))] pb-2">
        ...
        <h1 className="text-sm font-bold text-[#B57CFF] absolute left-1/2 -translate-x-1/2">Creator Payout</h1>
        <div className="w-8" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-10 space-y-4">
        {loading ? (
          <div className="flex justify-center py-10">
            <div className="w-6 h-6 border-2 border-[#6F2BFF] border-t-transparent rounded-full animate-spin" />
          </div>
        ) : (
          <>
            <div className="rounded-xl border border-[#6F2BFF]/25 bg-white/5 p-3 space-y-2">
              <div className="flex items-center gap-2 text-[#B57CFF] font-bold text-sm">
            ...
            <div className="rounded-xl border border-[#6F2BFF]/25 bg-white/5 p-3 space-y-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-[11px] font-bold ${methodType === 'bank' ? 'bg-[#6F2BFF] text-black' : 'bg-white/10 text-white'}`}
                >
                  Bank
                </button>
                <button
                  ...
                  className={`flex-1 py-2 rounded-lg text-[11px] font-bold ${methodType === 'paypal' ? 'bg-[#6F2BFF] text-black' : 'bg-white/10 text-white'}`}
                >
                  PayPal
                </button>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg bg-[#6F2BFF] text-black text-[12px] font-bold disabled:opacity-50"
              >
            ...
            <div className="rounded-xl border border-[#6F2BFF]/25 bg-white/5 p-3 space-y-2">
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg bg-white/10 border border-[#6F2BFF]/40 text-[#B57CFF] text-[12px] font-bold disabled:opacity-50"
              >
            ...
            {requests.length > 0 ? (
              <div className="rounded-xl border border-white/10 bg-white/5 p-3 space-y-2">
                <p className="text-[#B57CFF] font-bold text-sm">Your requests</p>
