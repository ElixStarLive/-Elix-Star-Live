...
export default function SavedVideos() {
  ...
  return (
    <div className="h-full min-h-0 w-full bg-[#0B0B0E] text-white flex justify-center px-2">
      <div className="w-full max-w-[480px] h-full min-h-0 flex flex-col overflow-y-auto bg-[#0B0B0E]">
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="w-10 h-10 border-3 border-[#5F0AE3] border-t-transparent rounded-full animate-spin" />
          </div>
        ) : loadError ? (
          ...
          <>
            <div className="grid grid-cols-3 gap-0.5 p-0.5 flex-1 overflow-y-auto">
              {videos.map((video) => (
                <div
                  key={video.id}
                  className="aspect-[3/4] bg-[#0B0B0E] relative cursor-pointer group"
