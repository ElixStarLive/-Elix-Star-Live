    ...
    <div className="min-h-screen bg-[#111113] text-white p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <h1 className="text-2xl font-bold mb-2 flex items-center gap-2">
          <DollarSign className="w-7 h-7 text-[#B57CFF]" />
          Purchases
        </h1>
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold ${
              tab === "iap" ? "bg-[#6F2BFF] text-black" : "bg-white/10"
            }`}
          >
            Coin IAP
          </button>
          <button
            ...
            onClick={() => setTab("shop")}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold ${
              tab === "shop" ? "bg-[#6F2BFF] text-black" : "bg-white/10"
            }`}
          >
            Shop (Stripe)
          </button>
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={String(r.id ?? i)} className="border-t border-white/5">
                    ...
                    <td className="p-2 tabular-nums text-[#B57CFF]">
                      {r.coins_delta != null
