...
export default function ChatThread() {
  ...
  if (isSystemThread) {
    return (
      <div className="min-h-full min-h-0 flex flex-col bg-[#121215] text-white p-4">
  ...
  return (
    <div
      className="fixed left-0 right-0 flex flex-col w-full max-w-[480px] mx-auto bg-[#121215] text-white z-[1]"
      style={{ top: 0, bottom: 'var(--bottom-ui-reserve)' }}
    >
        <header className="flex-shrink-0 flex items-center gap-2 px-4 py-3 border-b border-white/10 bg-[#121215]">
          <div className="flex w-12 shrink-0 items-center justify-start">
            {otherUser && (
              <button
                type="button"
                onClick={handleVideoCall}
                className="p-2 rounded-full bg-[#121215] border border-[#E5E5E7]/40 hover:bg-[#FF3B3F]/10 transition-colors"
        ...
        {liveUsers.length > 0 && (
          <div className="flex-shrink-0 border-b border-white/10 bg-[#121215]">
          ...
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm leading-snug break-words ${isMe ? 'bg-[#FF3B3F] text-black rounded-tr-none' : 'bg-[#1F1F26] text-white rounded-tl-none'}`}>
        ...
        <div className="flex-shrink-0 p-4 bg-[#121215] border-t border-white/10">
